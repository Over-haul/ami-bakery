...
MY_ROLE="$(aws ec2 describe-instances --region "${MY_REGION}" \
                                      --instance-ids "${MY_INSTANCE_ID}" \
                                      --query "Reservations[0].Instances[0].Tags[?Key==\`$(get_ivy_tag):role\`].{Value:Value}" \
                                      --output text)"
MY_SERVICE="$(aws ec2 describe-instances --region "${MY_REGION}" \
                                         --instance-ids "${MY_INSTANCE_ID}" \
                                         --query "Reservations[0].Instances[0].Tags[?Key==\`$(get_ivy_tag):service\`].{Value:Value}" \
                                         --output text)"
...
function run_backup() {
    ...
    EBS_DATA_VOLUME_ID="$(aws ec2 describe-instances --region "${MY_REGION}" \
                                                    --instance-ids "$(get_instance_id)" \
                                                    --query "Reservations[0].Instances[0].BlockDeviceMappings[?DeviceName==\`${DATA_DEVICE}\`].{id:Ebs.VolumeId}" \
                                                    --output text)"
    ...
    # Snapshot the volume
    SNAPSHOT_ID="$(aws ec2 create-snapshot --region "${MY_REGION}" \
                                           --volume-id "${EBS_DATA_VOLUME_ID}" \
                                           --description "${TIMESTAMP} ${MY_ROLE} Backup - Snapshot Name: ${SNAP_NAME}" \
                                           --output text \
                                           --query '{ID:SnapshotId,STATE:State,VOLUME:VolumeId}' | awk '{print $1}')"
    sleep 5

    # Tag it!
    aws ec2 create-tags --region "${MY_REGION}" \
                        --resources "${SNAPSHOT_ID}" \
                        --tags "Key=Name,Value=${MY_ROLE}" \
                               "Key=$(get_ivy_tag):role,Value=${MY_ROLE}" \
                               "Key=$(get_ivy_tag):sysenv,Value=$(get_sysenv)" \
                               "Key=$(get_ivy_tag):service,Value=${MY_SERVICE}"

    # Delete old snapshots
    CUTOFF_DAY=$(date -d 'now - 7 days' -u +"%Y-%m-%dT00:00:00.000Z")
    IFS=$'\n' && for snapshot in $(aws ec2 describe-snapshots --region "${MY_REGION}" \
                                                              --filters "Name=tag:$(get_ivy_tag):sysenv,Values=$(get_sysenv)" \
                                                                        "Name=tag:$(get_ivy_tag):service,Values=${MY_SERVICE}" \
                                                                        "Name=tag:$(get_ivy_tag):role,Values=${MY_ROLE}" \
                                                              --query 'Snapshots[*].[SnapshotId,StartTime]' \
                                                              --output text); do
            snapshot_id="$(awk '{print $1}' <<<"${snapshot}")"
            start_time="$(awk '{print $2}' <<<"${snapshot}")"
